...
await syncPendingStatusUpdates();
  ...
  Future<void> approveLeaveRequestOffline(String leaveRequestId) async {
    ...
    if (leaveRequest != null) {
      ...
      final pendingStatusUpdatesBox =
          await Hive.openBox<Map<String, dynamic>>('pendingStatusUpdatesBox');
      await pendingStatusUpdatesBox.put(leaveRequestId, {
        'id': leaveRequestId,
        'status': 'Approved',
      });
  ...
  Future<void> rejectLeaveRequestOffline(String leaveRequestId) async {
    final leaveRequestBox =
        await Hive.openBox<LeaveRequest>('leaveRequestsBox');
    final leaveRequest = leaveRequestBox.get(leaveRequestId);
  ...
  Future<void> syncPendingStatusUpdates() async {
    final pendingStatusUpdatesBox =
        await Hive.openBox<Map<String, dynamic>>('pendingStatusUpdatesBox');
    final pendingUpdates = pendingStatusUpdatesBox.values.toList();
    ...
    for (var update in pendingUpdates) {
      try {
        ...
        await pendingStatusUpdatesBox.delete(update['id']);
      } catch (error) {
        print(
            'Failed to sync status update for leave request ${update['id']}: $error');
      }
    }
  }

  Future<void> syncPendingDeletions() async {
    final pendingDeletionsBox =
    ...
    for (String leaveRequestId in pendingDeletions) {
      try {
        ...
        print(
            'Failed to delete leave request with ID: $leaveRequestId - $error');
      }
    }
  }
    ...
    try {
      ...
      for (var leaveRequest in offlineRequests) {
        try {
          ...
          print(
              'Error syncing leave request with ID ${leaveRequest.Id}: $error');
        }
      }
              ...
              children: [
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (isOffline) {
                      final leaveRequestBox =
                          Hive.box<LeaveRequest>('leaveRequestsBox');
                      final localLeaveRequest =
                          leaveRequestBox.get(workDetail['id']);
                      if (localLeaveRequest != null) {
                        statusText = localLeaveRequest.status;
                      }
                    }
                                      ...
                                      if (isOnline) {
                                        ...
                                        await leaveRequestBox.delete(leaveRequestId)
                                          ...
                                          ScaffoldMessenger.of(context)
                                            ...
                                            SnackBar(
                                              content: Text(
                                                  'Failed to delete leave request: $error'),
                                              backgroundColor: Colors.red,
                                            ),
                                          );
                                        });
  ...
  Future<void> showLeaveRequestDetailDialog(
      Map<String, dynamic> leaveRequest) async {
...
class LeaveRequestService {
  final Box<LeaveRequest> leaveRequestBox =
      Hive.box<LeaveRequest>('leaveRequestsBox');
